...
import { checkReAuthSession } from "../../auth/sessionManager-app.js";
                ...
                if(newEmail) {
                    updateEmail(auth.currentUser, newEmail).then(async () => {
                        ...
                        if(errorCode==='auth/requires-recent-login'){
                            window.location.href = `${setURLPath()}pages/user/login.html?re-auth=true&redirect=adminAccount`;
                        }
                        addAlert('FM-E-0003');
                    });
                }
        ...
        if(currentPassword!='' && newPassword!='' && confirmNewPassword!=''){
            if(newPassword === confirmNewPassword) {
                ...
                if(isReAuth && isSessionAvailable) {
                    ...
                    console.log('Please re-authenticate');
                    window.location.href = `${setURLPath()}pages/user/login.html?re-auth=true&redirect=adminSecurity`;
                }
            } else {
                addAlert('FM-D-0001');
            }
